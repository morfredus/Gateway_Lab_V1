...
- **Scan de ports** : détection des services actifs sur chaque équipement (HTTP, SSH, MQTT…)
- **Détection nouveaux équipements** : notification à l'apparition d'un MAC inconnu (log + API)
- **Endpoint `/api/export`** : téléchargement de l'inventaire en JSON
...
| v0.0.7 | **Résolution des noms d'hôtes** : mDNS passif + PTR DNS batch ; détection boxes FAI FR (Free/Orange/SFR/Bouygues) ; nouvelle `struct NetworkDevice` (category, model, os, source) ; badges source (mDNS / DNS↩ / ESP32) + modèle dans l'UI ; auto-inscription de l'ESP32 dans la liste |
